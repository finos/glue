...
CREATE TABLE IF NOT EXISTS `Instrument__Attribute_Value` (
  ...
  `Instrument_Attribut` BIGINT(12) NOT NULL,
  `Instrument` BIGINT(12) NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `Market_data` (
  `Id` BIGINT(12) NOT NULL,
  `Instrument_Attribute` BIGINT(12) NOT NULL,
  `Instrument` BIGINT(12) NOT NULL,
  `Value` VARCHAR(45) NULL,
  `Timestamp` TIMESTAMP(3) NULL,
  PRIMARY KEY (`Id`),
  CONSTRAINT `Instrument_Market`
    FOREIGN KEY (`Instrument`)
    REFERENCES `Instrument` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Attribute_Market`
...
CREATE INDEX `Instrument_Market_idx` ON `Market_data` (`Instrument` ASC) VISIBLE;

CREATE INDEX `Attribute_Market_idx` ON `Market_data` (`Instrument_Attribute` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `Node_CMA_RR` (
  ...
  CONSTRAINT `RRCMANode`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `RRCMA_Risk`
    FOREIGN KEY (`Risk_Type`)
    REFERENCES `Risk_Type` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE INDEX `RRCMA_Risk_idx` ON `Node_CMA_RR` (`Risk_Type` ASC) VISIBLE;
...
CREATE INDEX `Portfolio_Product_idx` ON `Portfolio` (`Product_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `Position` (
  ...
  `purchase_datetime` DATETIME(3) NOT NULL COMMENT 'Date when the position was created',
  ...
  `market_value_datetime` DATETIME(3) NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `Price` (
  `id` BIGINT(12) NOT NULL,
  `Instrument_id` BIGINT(12) NULL,
  `Currency_id` BIGINT(12) NULL,
  `Quote_type_id` BIGINT(12) NULL,
  `Value` DECIMAL(25,9) NULL,
  `Timestamp` TIMESTAMP(3) NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `Instrument_price`
    FOREIGN KEY (`Instrument_id`)
    REFERENCES `Instrument` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Currency_price`
    FOREIGN KEY (`Currency_id`)
    REFERENCES `Currency` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Type_price`
    FOREIGN KEY (`Quote_type_id`)
    REFERENCES `Price_Quote` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `Instrument_id_idx` ON `Price` (`Instrument_id` ASC) VISIBLE;

CREATE INDEX `Currency_price_idx` ON `Price` (`Currency_id` ASC) VISIBLE;

CREATE INDEX `Type_price_idx` ON `Price` (`Quote_type_id` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `Product_Hierarchy` (
  ...
  `Child` BIGINT(12) NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `Parent_Product`
    FOREIGN KEY (`Parent`)
    REFERENCES `Product` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `Child_Product`
    FOREIGN KEY (`Child`)
    REFERENCES `Product` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `Child_Product_idx` ON `Product_Hierarchy` (`Child` ASC) VISIBLE;

CREATE INDEX `Parent_Product_idx` ON `Product_Hierarchy` (`Parent` ASC) VISIBLE;
...
CREATE INDEX `Risk_Type_id_idx` ON `Risk_Profile` (`Risk_Type` ASC) VISIBLE;
